...
import { services, getServiceName, getServiceShortDescription } from "@/lib/data";
...
export default function SearchBar({ open, onClose }: Props) {
  ...
  const suggestions = useMemo(() => {
    ...
    return services
      .map(s => ({
        ...s,
        localName: getServiceName(s.slug, s.name, lang),
        localDesc: getServiceShortDescription(s.slug, s.shortDescription, lang)
      }))
      .filter(s =>
        s.localName.toLowerCase().includes(needle) ||
        s.localDesc.toLowerCase().includes(needle) ||
                    ...
                    <li key={s.slug}>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-primary text-sm">{s.localName}</p>
                          <p className="text-xs text-on-surface-variant truncate">{s.localDesc}</p>
                        </div>
