    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-[100] glass-header border-b border-outline-variant/30">
        ...
        <div className="max-w-container-max-width mx-auto px-3 md:px-margin-desktop h-20 flex flex-row-reverse items-center justify-between gap-2">
          ...
          {/* Desktop Navigation */}
          <nav className="hidden lg:flex flex-row-reverse items-center gap-8 xl:gap-10" aria-label="ניווט ראשי">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
          ...
          <div className="flex items-center gap-1.5 sm:gap-3">
            {/* Support button - icon-only on mobile, full on desktop */}
            <button
              type="button"
              ...
              aria-label="פתח טופס פנייה למוקד"
              data-tooltip='פנייה למוקד התמיכה של מפ"י - מענה תוך 1-2 ימי עסקים'
              data-tooltip-position="bottom"
            >
              <span className="material-symbols-outlined text-[20px]">support_agent</span>
              <span className="hidden md:inline">פנייה למוקד</span>
            </button>
